...
namespace Example
{
    class Program
    {
        ...
        private const string PfxCertificatePath = @"C:\Users\even.skari\Code\Heimdall\HP.Cloud.API-clients\DotNetClient\DotNetClient\c.pfx";
        private const string CertificatePassword = "";

        private static async Task Main(string[] args)
        {
            Console.WriteLine("Hello World!");
            var api = new HeimdallApiClient(ClientId, PfxCertificatePath, CertificatePassword);
            ...
            var measurementsLine = await api.GetAggregatedMeasurements(
                line,
                null,
                from,
                to,
                IntervalDuration.EveryHour,
                MeasurementType.Current,
                AggregationType.Average
            );
            var measurementsSpan = await api.GetAggregatedMeasurements(
                line,
                span,
